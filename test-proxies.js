...
async function main() {
  try {
    ...
    for (let i = 0; i < ipsToCheck.length; i += batchSize) {
      ...
      const totalBatches = Math.ceil(ipsToCheck.length / batchSize);
      ...
      console.log(`--- Batch ${batchNum} finished. Found ${successesInBatch} proxies. Total so far: ${workingProxies.length} ---`);
    }

    console.log(`All batches processed. Found a total of ${workingProxies.length} working proxies.`);
    generateMarkdown(workingProxies);
